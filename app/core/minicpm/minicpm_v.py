    ...
    def chat(self, input: ChatCompletionsRequest):
                            ...
                            if item["type"] == "text":
                                ...
                                if "url" in image_url:
                                    ...
                                    if splitted_url[0].startswith("data:image/"):
                                        image = Image.open(
                                            io.BytesIO(
                                                base64.b64decode(splitted_url[1])
                                            )
                                        ).convert("RGB")
        ...
        if not has_image_in_all_messages and user_message_index is not None:
            ...
            if isinstance(processed_messages[0]["content"], list):
                ...
                processed_messages[user_message_index]["content"] = [
                    processed_messages[user_message_index]["content"],
                    blank_image,
                ]
...
class MiniCPMV2_6:
    def __init__(self, model_path, multi_gpus=False) -> None:
        ...
        if multi_gpus:  # inference on multi-gpus
            ...
            device_map["llm.lm_head"] = (
                device_id  # first and last layer of llm should be in the same device
            )
            ...
            self.model = load_checkpoint_and_dispatch(
                model, model_path, dtype=torch.bfloat16, device_map=device_map
            )
            self.model.eval()
        else:
    ...
    def chat(self, input: ChatCompletionsRequest):
        ...
        if not has_image_in_all_messages and user_message_index is not None:
            blank_image = _create_blank_image()
            if isinstance(processed_messages[user_message_index]["content"], list):
                processed_messages[user_message_index]["content"].append(blank_image)
            else:
                processed_messages[user_message_index]["content"] = [
                    processed_messages[user_message_index]["content"],
